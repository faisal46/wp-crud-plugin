 ...
 * @return int|WP_Error
 */
function faisal_ac_insert_address( $args = [] ) {
    global $wpdb;

    if ( empty( $args['name'] ) ) {
        return new \WP_Error( 'no-name', __( 'You must provide a name.', 'faisal-academy' ) );
    }
    ...
    if ( isset( $data['id'] ) ) {
        ...
        faisal_ac_address_purge_cache( $id );
        ...
        if ( ! $inserted ) {
            return new \WP_Error( 'failed-to-insert', __( 'Failed to insert data', 'faisal-academy' ) );
        }

        faisal_ac_address_purge_cache();
 ...
 * @return array
 */
function faisal_ac_get_addresses( $args = [] ) {
    global $wpdb;
 ...
 * @return int
 */
function faisal_ac_address_count() {
    global $wpdb;
 ...
 * @return object
 */
function faisal_ac_get_address( $id ) {
    global $wpdb;
 ...
 * @return int|boolean
 */
function faisal_ac_delete_address( $id ) {
    global $wpdb;
 ...
 * @return void
 */
function faisal_ac_address_purge_cache( $book_id = null ) {
    $group = 'address';
